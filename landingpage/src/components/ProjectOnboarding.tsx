...
export const ProjectOnboarding: React.FC<ProjectOnboardingProps> = ({
  ...
  // Start from an explicitly empty graph. The onboarding flow must never
  // imply that sample nodes or model assignments were observed in this
  // project; users define their own version from real agent evidence.
  ...
  if (!project) {
    return (
      <div className="flex min-h-full items-center justify-center overflow-auto bg-[#050505] p-6 text-[#D7DADD]">
        <form onSubmit={submit} className="onboarding-empty w-full max-w-xl rounded-xl border border-white/[0.1] bg-[#0A0C0E] p-8 shadow-2xl">
  ...
  return (
    <div className="onboarding-page flex min-h-full flex-1 items-start justify-center overflow-auto bg-[#050505] p-6 text-[#D7DADD]">
      <div className="onboarding-content w-full max-w-4xl py-8">
        <div className="onboarding-header flex flex-wrap items-start justify-between gap-5"><div><p className="text-[10px] font-mono uppercase tracking-[0.2em] text-[#7D858C]">Agent setup · {project?.name}</p><h1 className="mt-3 text-3xl font-medium text-[#F2F3F4]">Bring a real agent into focus.</h1><p className="mt-3 max-w-2xl text-sm leading-relaxed text-[#8C949B]">TwineRun will not invent a graph or metrics. Complete the evidence path below, then optimization becomes available.</p></div><button onClick={onRefresh} className="flex items-center gap-2 rounded border border-white/[0.1] px-3 py-2 text-xs text-[#A0A5AA] hover:border-white/25 hover:text-white"><RefreshCw className="h-3.5 w-3.5" />Refresh state</button></div>
        ...
        <div className="onboarding-steps mt-8 grid gap-3 md:grid-cols-2">
          {steps.map((step, index) => <article key={step.title} className={`onboarding-step rounded-xl border p-5 ${step.done ? "border-white/[0.16] bg-[#0C1011]" : "border-white/[0.08] bg-[#090B0C]"}`}><div className="flex items-start justify-between gap-4"><div className="flex items-center gap-3"><span className={`onboarding-step-index flex h-7 w-7 items-center justify-center rounded-full border font-mono text-xs ${step.done ? "border-[#B9C1C6] bg-[#E4E7E9] text-[#050505]" : "border-white/15 text-[#7D858C]"}`}>{step.done ? <Check className="h-4 w-4" /> : `0${index + 1}`}</span><h2 className="text-sm font-medium text-[#F2F3F4]">{step.title}</h2></div>{nextAction === ["DEFINE_VERSION", "CONNECT_AGENT", "IMPORT_EVALS", "RUN_BASELINE"][index] && !step.done && <span className="text-[9px] font-mono uppercase tracking-wider text-[#C7CDD1]">Next</span>}</div><p className="mt-4 text-xs leading-relaxed text-[#8C949B]">{step.detail}</p>{step.action && !step.done && <button onClick={step.action} className="mt-5 flex items-center gap-2 text-xs text-[#D7DADD] hover:text-white">{step.label}<ArrowRight className="h-3.5 w-3.5" /></button>}</article>)}
        </div>
        {activeStep === "version" && <section className="mt-4 rounded-xl border border-white/[0.1] bg-[#090B0C] p-5"><div className="flex items-center justify-between"><div><h2 className="text-sm text-[#F2F3F4]">Define version and graph</h2><p className="mt-1 text-xs text-[#8C949B]">Use nodes from your agent, not a template. One node per line: <code>id | name | role | provider/model</code>. Edges use <code>from -&gt; to</code>.</p></div><button onClick={() => setActiveStep(null)} className="text-xs text-[#7D858C]">Close</button></div><div className="mt-4 grid gap-3 sm:grid-cols-2"><label className="text-[10px] uppercase tracking-wider text-[#7D858C]">Version<input value={version} onChange={(event) => setVersion(event.target.value)} className="mt-2 w-full rounded border border-white/[0.1] bg-[#050505] px-3 py-2 text-xs text-white" /></label><label className="text-[10px] uppercase tracking-wider text-[#7D858C]">Environment<select value={environment} onChange={(event) => setEnvironment(event.target.value)} className="mt-2 w-full rounded border border-white/[0.1] bg-[#050505] px-3 py-2 text-xs text-white"><option>STAGING</option><option>PROD</option></select></label></div><label className="mt-4 block text-[10px] uppercase tracking-wider text-[#7D858C]">Nodes<textarea value={nodesText} onChange={(event) => setNodesText(event.target.value)} placeholder="planner | Planner | planning | provider/model" rows={5} className="mt-2 w-full rounded border border-white/[0.1] bg-[#050505] p-3 font-mono text-xs leading-relaxed text-white placeholder:text-[#515960]" /></label><label className="mt-4 block text-[10px] uppercase tracking-wider text-[#7D858C]">Edges<textarea value={edgesText} onChange={(event) => setEdgesText(event.target.value)} placeholder="planner -> researcher" rows={3} className="mt-2 w-full rounded border border-white/[0.1] bg-[#050505] p-3 font-mono text-xs leading-relaxed text-white placeholder:text-[#515960]" /></label>{formError && <p className="mt-3 text-xs text-[#D58C8C]">{formError}</p>}<button onClick={() => void saveVersion()} disabled={formBusy} className="silver-btn-gradient mt-4 rounded px-4 py-2 text-xs font-medium text-[#050505] disabled:opacity-40">{formBusy ? "Saving version…" : "Save version"}</button></section>}
