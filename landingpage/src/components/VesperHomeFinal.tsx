...
export const VesperHomeFinal: React.FC<Props> = ({ onLaunchStudio }) => {
  ...
  return <div className="vesper-page vesper-home-page"><div className="vesper-grain" aria-hidden="true" /><div className="vesper-photo vesper-photo--static" aria-hidden="true" /><div className="vesper-backdrop" aria-hidden="true" onClick={close} />
    <header className="vesper-header vesper-header--long vesper-home-header"><a href="/" className="vesper-logo vesper-reveal vesper-reveal--scale" onClick={close} aria-label="TwineRun home"><LogoMark /></a><nav className="vesper-nav" aria-label="Primary navigation">{links.map(([id, label]) => <a key={id} href={`/${id}`} className="vesper-nav-link" onClick={close}>{label}</a>)}</nav><div className="vesper-header-actions"><a className="vesper-btn vesper-btn--solid vesper-header-cta" href="/signin?returnTo=%2Fstudio">Launch TwineRun</a><button className="vesper-burger" onClick={() => setMenuOpen((open) => !open)} aria-controls="vesper-home-mobile-nav" aria-expanded={menuOpen} aria-label={menuOpen ? 'Close menu' : 'Open menu'}>{menuOpen ? <X size={18} /> : <Menu size={18} />}</button></div></header>
    <nav id="vesper-home-mobile-nav" className="vesper-mobile-nav" aria-label="Mobile primary navigation">{links.map(([id, label]) => <a key={id} href={`/${id}`} className="vesper-mobile-link" onClick={close}>{label}</a>)}<a className="vesper-btn vesper-btn--solid vesper-mobile-cta" href="/signin?returnTo=%2Fstudio">Launch TwineRun <ArrowRight size={15} /></a></nav>
    <main className="vesper-home-main" id="top"><section className="vesper-hero vesper-hero--home"><div className="vesper-copy"><div className="vesper-badge vesper-reveal vesper-reveal--pop">PROFILE-GUIDED OPTIMIZATION FOR AI AGENTS</div><h1><span className="vesper-headline-line vesper-reveal vesper-reveal--mask">Make your agents cheaper.</span><span className="vesper-headline-line vesper-reveal vesper-reveal--mask"><em>Without making them worse.</em></span></h1><p className="vesper-lede vesper-reveal vesper-reveal--soft">TwineRun profiles your agent, tests cheaper model configurations against your evals, and returns the lowest-cost execution plan that stays inside your quality bounds.</p><div className="vesper-actions"><a className="vesper-btn vesper-btn--solid vesper-hero-btn vesper-reveal vesper-reveal--btn" href="/signin?returnTo=%2Fstudio">Optimize an Agent <ArrowRight size={15} /></a></div><p className="vesper-proof-line">Offline optimization <span>·</span> No production changes without approval</p></div></section></main>
  </div>;
};
